...
def voxelize(
    ...
    if "labels" in input_dict:
        ...
        for i , sample in enumerate(batch):
            clip_feat = sample[-3][unique_maps[i]]
            clip_feat = torch.nn.functional.normalize(clip_feat, dim=-1, eps=1e-5).float()
            clip_feats.append(clip_feat)

            mva_feat = []
            gt_masks = target[i]['masks']
            for gt_mask in gt_masks:
                mva_feat.append(clip_feat[gt_mask].mean(dim=0).unsqueeze(0))
            mva_feats.append(torch.cat(mva_feat, dim=0))
            mca_feats.append(torch.cat(sample[-2]["features"], dim=0))
            mea_feats.append(sample[-1]["guide"])
    else:
        target = []
        target_full = []
        coordinates = []
