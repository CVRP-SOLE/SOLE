...
class SemanticSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        self.mca_feat_path = mca_feat_path
        self.mea_feat_path = mea_feat_path
    ...
    def __getitem__(self, idx: int):
        ...
        file_name = self.data[idx]["raw_filepath"].split("/")[-2]
        clip_feat = torch.load(
            os.path.join(self.clip_feat_path, file_name + "_0.pt"), map_location='cpu'
        ).float()
        with open(os.path.join(self.mca_feat_path, file_name + ".pickle"), "rb") as f:
            mca_feat = pickle.load(f)
        with open(os.path.join(self.mea_feat_path, file_name + ".pickle"), "rb") as f:
            mea_feat = pickle.load(f)
