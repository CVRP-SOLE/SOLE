...
class InstanceSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        weight_dict = {
            "loss_nel_focal": matcher.cost_nel_focal,
            "loss_nel_dice": matcher.cost_nel_dice,
    ...
    def training_step(self, batch, batch_idx):
        data, target, file_names, clip_feats, mva_feats, mca_feats, mea_feats = batch
        ...
        logs["train_mean_loss_nel_focal"] = statistics.mean(
            [item for item in [v for k, v in logs.items() if "loss_nel_focal" in k]]
        )
        logs["train_mean_loss_nel_dice"] = statistics.mean(
            [item for item in [v for k, v in logs.items() if "loss_nel_dice" in k]]
        )
    ...
    def eval_instance_step(
        ...
        # Pooling CLIP features for ensemble inference
        clip_feat = clip_feats[self.decoder_id]
        pooling_mask = output["pred_masks"][self.decoder_id]

        Q = pooling_mask.shape[-1]
        pooled_feat = []
        for i in range(Q):
            pooling_mask_ = (pooling_mask[:, i]).softmax(dim=0)
            pooling_mask_ = repeat(pooling_mask_, 'Q -> Q D', D = clip_feat.shape[-1])
            feat = clip_feat * pooling_mask_
            feat = feat.sum(dim=0)
            pooled_feat.append(feat)
        pooled_feat = torch.stack(pooled_feat, dim=0).unsqueeze(0).float()
        pooled_feat = torch.nn.functional.normalize(pooled_feat, dim=-1)

        class_cost = pooled_feat @ self.model.text_embed.t() * 100
        bg_cost = pooled_feat @ self.model.bg_feature.t() * 100
        clip_cost = torch.cat((class_cost, bg_cost), dim=-1)
        
        # cost ensemble
        clip_cost = torch.functional.F.softmax(clip_cost, dim=-1)[..., :-1]
        model_cost = torch.functional.F.softmax(
        ...
        # new method
        idx = clip_cost > model_cost
        lambda_balance = torch.ones_like(clip_cost)

        lambda_balance[idx == 1] = 2 / 3.
        lambda_balance[idx != 1] = 1 / 3.
        
        prediction[self.decoder_id][
            "pred_logits"
        ] = torch.pow(clip_cost, lambda_balance) * torch.pow(model_cost, (1 - lambda_balance))
    ...
    def eval_instance_epoch_end(self):
        ...
        try:
            ...
            if self.validation_dataset.dataset_name == "scannet200":
                ...
                ap_results[
                    f"{log_prefix}_mean_tail_ap"
                ] = mean_tail_results[0]
                ap_results[
                    f"{log_prefix}_mean_common_ap"
                ] = mean_common_results[0]
                ap_results[
                    f"{log_prefix}_mean_head_ap"
                ] = mean_head_results[0]
    ...
    def test_epoch_end(self, outputs):
        ...
        dd["val_mean_loss_nel_focal"] = statistics.mean(
            [item for item in [v for k, v in dd.items() if "loss_nel_focal" in k]]
        )
        dd["val_mean_loss_nel_dice"] = statistics.mean(
            [item for item in [v for k, v in dd.items() if "loss_nel_dice" in k]]
        )
