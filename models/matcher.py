...
class HungarianMatcher(nn.Module):
    ...
    def __init__(
        self,
        cost_class: float = 1,
        cost_nel_focal: float = 1,
        cost_nel_dice: float=1,
        cost_mask: float = 1,
        cost_dice: float = 1,
        num_points: int = 0,
    ):
        ...
        self.cost_nel_focal = cost_nel_focal
        self.cost_nel_dice = cost_nel_dice
    ...
    def memory_efficient_forward(self, outputs, targets, mask_type):
        ...
        bs, num_queries = len(targets), outputs["pred_mva_logits"][0].shape[0]

        indices = []

        # Iterate through batch size
        for b in range(bs):
            out_mva_prob = outputs["pred_mva_logits"][b].softmax(
                -1
            )[:, :-1]
            out_mca_prob = outputs[ "pred_mca_logits"][b].softmax(
                -1
            )[:, :-1]
            out_mea_prob = outputs["pred_mea_logits"][b].softmax(
                -1
            )[:, :-1]
            out_class_prob = (out_mva_prob + out_mca_prob + out_mea_prob) / 3.

            cost_class = -out_class_prob
