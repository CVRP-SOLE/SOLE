...
class SetCriterion(nn.Module):
    ...
    def loss_labels_nel(self, outputs, targets, indices, num_masks, mask_type):
        loss_masks = []
        loss_dices = []

        for logit_key in ["pred_mva_logits", "pred_mca_logits", "pred_mea_logits"]:
            for i, logit in enumerate(outputs[logit_key]):
                src_logit = logit.float().unsqueeze(0)
                idx = self._get_src_permutation_idx_train(indices[i])

                num_classes = src_logit.shape[2] - 1
                target_classes = torch.full(
                    src_logit.shape[:2],
                    num_classes,
                    dtype=torch.int64,
                    device=src_logit.device,
                )
                
                target_classes[idx] = indices[i][1].to(src_logit.device)

                target_one_hot = F.one_hot(target_classes, num_classes=num_classes+1).float()

                loss_masks.append(sigmoid_focal_loss(src_logit[0], target_one_hot[0], len(target_classes[0])))
                loss_dices.append(dice_loss_jit(src_logit[0], target_one_hot[0], len(target_classes[0])))

        return {
            "loss_nel_focal": torch.sum(torch.stack(loss_masks)),
            "loss_nel_dice": torch.sum(torch.stack(loss_dices)),
        }
    ...
    def _get_src_permutation_idx_train(self, indice):
        batch_idx = torch.full_like(indice[0], 0)
        src_idx = indice[0]
        return batch_idx, src_idx
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks, mask_type):
        loss_map = {"labels": self.loss_labels_nel, "masks": self.loss_masks}
    ...
    def forward(self, outputs, targets, mask_type):
        ...
        num_masks = torch.as_tensor(
            [num_masks],
            dtype=torch.float,
            device=list(outputs.values())[1][0].device,
        )
